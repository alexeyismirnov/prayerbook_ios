...
class SynaxarionModel : EbookModel {
    ...
    func forDate(_ date: Date) -> [Preachment] {
        ...
        dates = [
            ...
            cal.pascha-5.days,cal.pascha-4.days,cal.pascha-3.days,cal.pascha-2.days, cal.pascha-1.days]
        
        if (lang == "en") {
            dates.append(contentsOf: [
                cal.pascha, cal.pascha+7.days,cal.pascha+14.days,
        ...
        if let index = dates.firstIndex(of: date) {
            let pos = BookPosition(model: self, index: IndexPath(row: index, section: 0))
            return [Preachment(position: pos, title: getTitle(at: pos)!, subtitle: Translate.s("synaxarion_annotation"))]
