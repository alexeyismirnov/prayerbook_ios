...
enum DayContentBuilder {
    static func previewEntry() -> CalendarEntry {
        CalendarEntry(
            ...
            weekToneFasting: "Пост • глас",
            saint: SaintLine(name: "Православный календарь", isGreat: false, feastType: .none),
            monthTitle: "Календарь",
            weekdaySymbols: ["П", "В", "С", "Ч", "П", "С", "В"],
            monthCells: []
        )
    }
    ...
    private static func build(selectedDate: Date) throws -> CalendarEntry {
        ...
        let symbols = calLocale.veryShortWeekdaySymbols ?? ["S", "M", "T", "W", "T", "F", "S"]
    ...
    /// New-style / old-style church date, e.g. "30/17 окт." or "1 нояб./19 окт.".
    ...
    static func dualChurchDateTitle(_ date: Date) -> String {
        ...
        return "\(weekday) \(dual) \(date.year) г."
    }
